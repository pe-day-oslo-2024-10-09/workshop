...
PE_DAY_ORG="$(humctl get orgs -o json | jq -r '[.[] | select(.metadata.id | startswith("pe-day-osl^C")) | .metadata.id][0]')"
if [ "$PE_DAY_ORG" == "null" ]
then
...
AWS_SECRET_MANAGER_POLICY="$(aws iam list-policies | jq -r '.Policies[] | select(.PolicyName | startswith("secrets-manager-access-")) | .Arn')"
